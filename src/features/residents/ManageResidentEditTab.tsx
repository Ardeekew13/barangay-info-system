...
const ManageResidentEditTab: React.FC<Props> = ({
	form,
	residentId,
	occupation,
	setOccupation,
	...
	const { households: householdList, loading: householdLoading } =
		useHouseholdOptions();
	...
	const middleName = Form.useWatch("middle_name", form);
	const lastName = Form.useWatch("last_name", form);
	const currentResidentName = [firstName, middleName, lastName]
		.filter(Boolean)
		.join(" ");
	...
	const handleHouseholdCreated = (
		...
		if (householdId && householdLabel) {
			form.setFieldsValue({
				household: { value: householdId, label: householdLabel },
			});
		}
	};

	const handleOccupationCreated = (occupationName?: string) => {
	...
	return (
		...
		>
			...
			<Row gutter={[16, 2]}>
				...
				<Col {...responsiveColumn3}>
					<Form.Item label="Household" style={{ marginBottom: 0 }}>
						<Space.Compact style={{ width: "100%" }}>
			...
			<AddHouseholdFromEditModal
				...
				onClose={handleHouseholdCreated}
			/>

			<AddOccupationFromEditModal
				open={showOccupationModal}
				onClose={handleOccupationCreated}
			/>
...
export default ManageResidentEditTab;
